function [iter_in] = simulateForeignMatchesInnerSimNoLearners_nolearning(no_learn, stay, mm, iter_in, policy)
    ...
    if N_no_learn >0
        ...
        iter_in.cum_meets(no_learn,iter_in.t) = (iter_in.cum_meets(no_learn,iter_in.t-1)+ poissinv(rand(N_no_learn,1),...
            reshape(policy.lambda_f(floor(mm.theta2(mm.pt_type(iter_in.pt_ndx,2))*(mm.n_size+1)),(mm.n_size+1),mm.pt_type(iter_in.pt_ndx,2),...
            min((mm.net_size+1),max(iter_in.cum_succ(no_learn,iter_in.t-1)+1,mm.n_size+1)),...
