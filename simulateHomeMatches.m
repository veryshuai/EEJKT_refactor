...
for t = 2:1:mm.periods
    ...
    if  mm.sim_firm_num_by_prod_succ_type(pt_ndx) > 0
        ...
        for ii = 1:size(theta1_cntr,1)
            ...
            trans_rands(cntr+1:cntr2,:) = pmat_cum_ht(iterH_in.micro_state(cntr+1:cntr+theta1_cntr(ii,2),t-1),:)> rand(theta1_cntr(ii,2),1)*ones(1,mm.nn_h);
            cntr = cntr2;
    ...
    drop_cnt = sum(iterH_in.lag_cli_zst.*(1-keep_cli),2);
    ...
    if sum(ddum)>0
        iterH_in.exog_deaths(ddum,t-1) =...
            random('bino',iterH_in.cur_cli_cnt(ddum,t-1)-drop_cnt(ddum),(1-exp(-mm.delta)));
    end
    ...
    iterH_in.cur_cli_cnt(:,t) = iterH_in.add_cli_cnt(:,t) + iterH_in.cur_cli_cnt(:,t-1) ...
        - drop_cnt - iterH_in.exog_deaths(:,t-1) ;
    ...
    for i=1:mm.sim_firm_num_by_prod_succ_type(pt_ndx)
        ...
        if iterH_in.exog_deaths(i,t-1) > 0
            ...
            iterH_in.die_cli_zst(i,:) = createDieVec(iterH_in.lag_cli_zst(i,:).*keep_cli,iterH_in.exog_deaths(i,t-1),size(mm.Z,1));
            % record number of endogenous plus exogenous exits by z state in 1st column of trans_count:
        end
        iterH_in.trans_count(2:size(mm.Z,1)+1,1,i) = (iterH_in.lag_cli_zst(i,:).*(1-keep_cli))' + iterH_in.die_cli_zst(i,:)';
        ...
        iterH_in.surviv_zst(i,:) = iterH_in.lag_cli_zst(i,:).*keep_cli - iterH_in.die_cli_zst(i,:);
    ...
    flr       = max(iterH_in.flrlag,t*iterH_in.new_firm(:,t));  % floor resets to current year for new firms. Hence
    ...
    iterH_in.seas_tran{1,season} = [[t,season,year].*ones(size(mat_tran,1),1),mat_tran,ship_cur,age_vec];
    iterH_in.seas_Zcut(season)   = drop_Zcut;
    ...
    if season == mm.pd_per_yr
        ...
        [~,firm_yr_sales] =...
            season_merge(iterH_in.seas_tran,N_match,mm.sim_firm_num_by_prod_succ_type(pt_ndx),mm.pd_per_yr);
