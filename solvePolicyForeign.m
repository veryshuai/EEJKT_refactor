...
for prod_ind = 1:size(mm.Phi)
    for succ = 1:mm.n_size+1
        ...
        perm_flow = policy.postSuccessProb_f(mm.n_size+1,succ)*squeeze(l_opt(succ,mm.n_size+1,1,mm.net_size+1,prod_ind,:)).*policy.pi_f(prod_ind,:)'-mm.cost_f(squeeze(l_opt(succ,mm.n_size+1,1,mm.net_size+1,prod_ind,:)),mm.net_size+1);
        ...
        for m = 1:mm.net_size+1-succ
            ...
            new_v = val_func_iter('no_learning',squeeze(V(succ,mm.n_size+1,1,mm.net_size+2-m,prod_ind,:)),squeeze(V(succ,mm.n_size+1,1,mm.net_size+2-m,prod_ind,:)),[],policy.postSuccessProb_f(mm.n_size+1,succ),policy.pi_f(prod_ind,:),mm.net_size+1-m,mm,policy,'foreign');
            ...
            [~,l_opt(succ,mm.n_size+1,1,mm.net_size+1-m,prod_ind,:)] = iterationError_h(squeeze(V(succ,mm.n_size+1,1,mm.net_size+1-m,prod_ind,:)),...
                policy.postSuccessProb_f(mm.n_size+1,succ),policy.pi_f(prod_ind,:),mm.net_size+1-m,squeeze(V(succ,mm.n_size+1,1,mm.net_size+2-m,prod_ind,:)),mm,policy,'foreign');
