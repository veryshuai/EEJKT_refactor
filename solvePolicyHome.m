...
for prod_ind = 1:size(mm.Phi)
    for k = 1:mm.dim1
        ...
        l_opt(1,k,mm.net_size+1,prod_ind,:) = mm.l_opt_func_h(mm.theta1(k),mm.net_size+1,policy.pi_h(prod_ind,:),0,0,0);
        ...
        for N = 1:mm.net_size
            ...
            [~,l_opt(1,k,mm.net_size+1-N,prod_ind,:)] = iterationError_h(squeeze(V(1,k,mm.net_size-N+1,prod_ind,:)),mm.theta1(k),policy.pi_h(prod_ind,:),mm.net_size+1-N,squeeze(V(1,k,mm.net_size-N+2,prod_ind,:)),mm,policy,'home');
