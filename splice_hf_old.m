function sim_out = splice_hf_old(sim_out,policy,mm)
...
obs_id_h=sortrows(obs_id_h);
obs_id_f=sortrows(obs_id_f);
d_match_h = logical([1;obs_id_h(2:end)-obs_id_h(1:end-1)~=0]);
d_match_f = logical([1;obs_id_f(2:end)-obs_id_f(1:end-1)~=0]);


obs_id_h2 = obs_id_h(d_match_h);
obs_id_f2 = obs_id_f(d_match_f);
assert(length(unique(obs_id_h2)) == length(obs_id_h2));
assert(length(unique(obs_id_f2)) == length(obs_id_f2));
